...
//Note: In the offset() method there is the theoretical risk of an integer overflow
//as we are multiplying two int values together. However, this is mitigated by
//the validation rules we created in our ValidateFilters() function, where we enforced
//maximum values of page_size=100 and page=10000000 (10 million). This means
//that the value returned by offset() should never come close to overflowing.
...
func calculateMetadata(totalRecords, page, pageSize int) Metadata {
	...
	//So, for example, if there were 12 records in total
	// and a page size of 5, the last page value would be (12+5-1)/5 = 3.2,
	//or 12%5 != 0, then int(12%5) +1
	return Metadata{
		CurrentPage:  page,
		PageSize:     pageSize,
		FirstPage:    1,
